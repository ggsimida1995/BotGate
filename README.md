...
For Nginx inline mode, do not copy a panel or ServBay path into the app bundle. Bot Gate automatically checks `PATH`, common install locations, and the running `nginx.exe` process on Windows. An empty `vhost_dir` makes Bot Gate discover the active files with `nginx -T`, and `config_file = ""` uses Nginx's default configuration. `include_dir` is relative to the Bot Gate config (default `data/nginx`). The dashboard only exposes absolute paths under advanced settings for a stopped Nginx with a fully custom install; users do not configure paths for each site. The process must have permission to read and update the loaded vhost and reload Nginx.
