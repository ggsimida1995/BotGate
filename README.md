...
Windows releases are distributed as a single installer that places `bot-gate.exe`, `config.toml`, and `frontend/dist` together. macOS releases are distributed as a `.dmg` containing a complete `.app` bundle with the same resources. Double-clicking the installed Windows executable starts the loopback management service and leaves a Bot Gate icon in the system tray; the tray menu opens the management dashboard or exits the process. On macOS, the app bundle copies its default configuration and runtime data to the user's Application Support directory on first launch.
