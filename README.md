...
For Nginx inline mode, do not copy a panel or ServBay path into the app bundle. Bot Gate automatically checks `PATH`, common install locations, and the running `nginx.exe` process on Windows. An empty `vhost_dir` makes Bot Gate discover the active files with `nginx -T`, and `config_file = ""` uses Nginx's default configuration. `include_dir` is relative to the Bot Gate config (default `data/nginx`). If automatic detection is not suitable, use **设置 Nginx 路径** beside **添加站点** and enter the Nginx installation directory or executable path once; the setting is persisted in SQLite and reused for all sites. The process must have permission to read and update the loaded vhost and reload Nginx.
