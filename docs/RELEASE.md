...
发布包启动后会自动运行 Bot Gate 内部网关，同时保留本机管理端口。默认网关使用回环动态端口，Nginx 内联模式会把 include 指向本次实际绑定的端口，不会占用业务站点端口；独立前置代理模式需要在 `[server].listen` 中明确设置公网端口。管理台仍可手动停止或重新启动网关。启用许可证时，网关启动接口会先验证本地许可证文件和签名有效期；许可证无效时只保留管理台，网关不会监听。
